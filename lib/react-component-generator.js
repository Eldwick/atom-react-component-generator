...
const insertRenderCollections = (subcomponents, template) => {
  return subcomponents.reduce((memo, { name, isCollection }) => {
    ...
    return indexOfRender === -1
      ? insertIntoFuncTemplate({ memo, name })
      : insertIntoTemplate({ memo, indexOfRender, renderMethod })
  }, template)
}


const insertIntoFuncTemplate = ({ memo, name }) => {
  const indexOfRender = memo.indexOf("const")
  const renderMethod = `const render${name}s = () => {\n  return COLLECTION.map(ELEMENT => <${name} key={} />)\n}\n\n`
  return insertIntoTemplate({ memo, indexOfRender, renderMethod })
}

const insertIntoTemplate = ({ memo, indexOfRender, renderMethod }) => (
  memo.slice(0, indexOfRender) + renderMethod + memo.slice(indexOfRender)
)
